...
\begin{definition}
	...
	$$
	\exists U \in \mathcal T : A \subseteq U \subseteq N.
	$$
...
\begin{definition}
	...
	$$
	\exists k \in \mathbb N : u[\mathbb N_{> k}] \subseteq U.
	$$
\end{definition}
%--------------------------------

% todo: this is wrong!!!!!
% see: https://proofwiki.org/wiki/Definition:Convergent_Sequence/Topology
% todo: Share on ProofWiki!!!!!!!
% this is just a proof hint, rewrite it!
%--------------------------------
\begin{proposition}
	Let $\mathbb X = (X, \mathcal T)$ be a topological space, and let $u: \mathbb N \to X$.
	
	$u$ converges to a limit $x \in X$ iff for any $U \in \mathcal T$ with $x \in U$, $U \cap u[\mathbb N]$ is a cofinite subset of $u[\mathbb N]$.
	
	\begin{proof}
		First, prove $\Rightarrow$.
		
		As $u$ converges to $x$, then, for any $U \in \mathcal T$ with $x \in U$,
		$$
		\exists k \in \mathbb N : u[\mathbb N_{> k}] \subseteq U. \qquad \text{(Definition \ref{def: convergent sequence})}
		$$
		
		As $\mathbb N_{> k}$ is a cofinite subset of $\mathbb N$, $u[\mathbb N_{> k}]$ is a cofinite subset of $u[\mathbb N]$.
		
		Also, we have
		$$
		u[\mathbb N] \setminus U \subseteq u[\mathbb N \setminus \mathbb N_{> k}].
		$$
		
		As $|u[\mathbb N \setminus \mathbb N_{> k}]|$ is finite, $U \cap u[\mathbb N]$ is a cofinite subset of $u[\mathbb N]$.
		
		\qedlm
		
		Now, prove $\Leftarrow$.
		
		Aiming for a contradiction, suppose $u$ does not converges to $x$.
		
		By Definition \ref{def: convergent sequence}, there exists $U \in \mathcal T$ with $x \in U$, such that
		$$
		\forall k \in \mathbb N: u[\mathbb N_{> k}] \setminus U \ne \emptyset.
		$$
		
		Suppose $u[\mathbb N_{> k}] \setminus U$ is finite, then there exists $m \in \mathbb N_{> k}$ such that $u[\mathbb N_{> m}] \subseteq U$. But this implies that $u[\mathbb N_{> m}] \setminus U = \emptyset$, contradicting to the assumption, thus $u[\mathbb N_{> k}] \setminus U$ has to be infinite.
		
		Therefore, there exists $U \in \mathcal T$ with $x \in U$, such that $u[\mathbb N_{> k}] \setminus U$ is infinite. This contradicts that for any $U \in \mathcal T$ with $x \in U$, $U \cap u[\mathbb N]$ is a cofinite subset of $u[\mathbb N]$.
		
		Thus $u$ has to converge to $x$.
...
\begin{proposition}
	...
	\begin{proof}
		As $x$ is a limit of $u$, by Definition \ref{def: convergent sequence}, for any $U \in \mathcal T$ with $x \in U$, $U \cap u[\mathbb N]$ is a cofinite subset of $u[\mathbb N]$
	\end{proof}
\end{proposition}
%--------------------------------
