...
\begin{definition}
	...
	Let $\mathbb X = (X, \mathcal T_X)$ and $\mathbb Y = (X, \mathcal T_Y)$, let $f: \mathbb X \to \mathbb Y$, and let $A \subseteq X$ be a mapping.
	
	Then $f$ is said to be \textit{continuous on $A$} iff for any $U_X \in \mathcal T_X$ with $A \subseteq U_X$,
	$$
...
\begin{note}
	...
	For example, let $\mathbb X = (\mathbb R, \mathcal T_X)$ be a topological space where $\mathcal T$ induced by Euclidean metric, and let $\mathbb Y = (\mathbb R, \mathcal T_Y)$ be a discrete topological space. Let $\imath: \mathbb X \to \mathbb Y$ be an inclusion mapping; i.e., it is defined as
	$$
	\imath: \mathbb X \to \mathbb Y : x \mapsto x.
	$$
	
	For any $A \subseteq \mathbb R$, clearly, $\imath[A] \in \mathcal T_Y$ holds. But for some (or for all) $B \in \mathcal T_Y \setminus \mathcal T_X$, $\imath^{-1}[B] \notin \mathcal T$. Thus, $\imath$ is not a continuous mapping.
\end{note}
%--------------------------------
