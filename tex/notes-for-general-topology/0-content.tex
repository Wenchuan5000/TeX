...
\begin{theorem}
	...
	\begin{proof}
		...
		\begin{aligned}
			% line 1
			A \subseteq B &\iff A^\complement \supseteq B^\complement
				% by
				& \\
			% line 2
			&\textcolor{red}{\implies} \left(A^\complement \right)^\circ \supseteq \left( B^\complement \right)^\circ
				% by
				&\text{(Theorem \ref{thm: inclusion implies interior inclusion})} \\
			% line 3
			& \iff \left( \left( A^\complement \right)^\circ \right)^\complement \subseteq \left( \left(B^\complement \right)^\circ \right)^\complement
				% by
				\\
			& \iff  
		\end{aligned}
